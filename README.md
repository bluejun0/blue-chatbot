...
| 이름 | 기본값 | 설명 |
|---|---|---|
| `FAQ_DIR` | `data/faq` | 도메인별 FAQ 파일이 있는 경로 |
...
`data/faq` 안의 yaml을 수정하고 서버를 재시작한다. 파일 이름이 도메인 이름이 되므로
도메인을 추가할 때는 `data/faq/이름.yaml`을 만들기만 하면 된다.

`id`는 도메인이 달라도 전체에서 고유해야 한다. 답변이 근거로 삼은 항목을 `matched_id`
하나로 가리키기 때문이다. 중복이나 누락이 있으면, 또는 파일이 하나도 없으면 기동에 실패한다.
